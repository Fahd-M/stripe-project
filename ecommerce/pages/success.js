...
const Success = () => {
    const { setCartItems, setTotalPrice, setTotalQuantities } = useStateContext(); // will set these all to 0

    //runs when the page loaded initially.Note: dependency array is empty(second argument).
    useEffect(() => {
        localStorage.clear();
        setCartItems([]);
        setTotalPrice(0);
